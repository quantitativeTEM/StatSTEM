...
if isempty(limits)
    limits = {'Limits',[get(ha,'XLim'),get(ha,'YLim')]};
    set(ha,'UserData',limits)
end
...
set(h.fig,'WindowButtonMotionFcn',@changepointer,'WindowButtonDownFcn',@getStat,'WindowButtonUpFcn',@updateWindow,'WindowScrollWheelFcn',@scroll_zoom,'WindowKeyReleaseFcn',@key_zoom);
...
if restart==0
    ...
    function updateWindow(hf,~)
        ...
        pntr2 = get(hf,'Pointer');
        button = get(hf, 'SelectionType');
        if (strcmp(pntr2, 'custom'))
            if strcmp(button,'normal')
