function panel = panelAtomCountStat()
% panelAtomCountStat - Create panel with buttons for counting atoms in the statistical framework
%
%   syntax: panel = panelAtomCountStat()
...
panel.name = 'Atom counting - Statistical';
...
panel.row(1).option1 = pushbutton('name','Run ICL','width',162,'func','fitGMM','input','output','output','atomcounting','fitting',1,...
    'figEnd','Observation','figOptEnd',{'Atom Counts'},'reshowFigEnd',true);

panel.row(2).option1 = pushbutton('name','Create 3D model','width',162,...
    'input',{'atomcounting.Counts','strainmapping.indices'},'output','model3D',...
...
panel.showAdv = 0;
panel.advanced(1).panel = panelPreCountStat;
panel.advanced(2).panel = panelPostCountStat;