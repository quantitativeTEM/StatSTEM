function output = combinedLorentz(output, K, L, ind)
...
for i=1:N
    ind_s = selectSectionInd(indM, rho(i)/output.dx*10, betaX(i)/output.dx+0.5, betaY(i)/output.dx+0.5);
    R2 = ( ( X(ind_s) - betaX(i) ).^2 + (Y(ind_s) - betaY(i)).^2);
    model(ind_s) = model(ind_s) + eta(i)*(rho(i)^2+R2).^(-3/2);
end
output.pModel = model;