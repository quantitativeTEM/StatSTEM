function Fun = criterionRhoLo(rho,Xreshape,Yreshape,K,L,reshapeobs,betaX,betaY,fitzeta,type,indMat,dx,peakShape,ind,max_n,GUI,hwaitbar,offwait,maxwait,maxIter)
% criterionRho - criterion to fit the width of Lorentzian peaks
%
%   Criterion is used to fit only the width of Lorentzian peaks. The width is
%   the same for each atom type.
%
%   syntax: Fun = criterionRhoLo(rho,Xreshape,Yreshape,K,L,reshapeobs,betaX,betaY,fitzeta,type,indMat,dx,peakShape,ind,max_n,hwaitbar,abortbut,offwait,maxIter)
...
rho_vec = rho(type);
Lo = sparse(K*L,length(type)+fitzeta);
Lo(:,1:end-fitzeta) = getLo(K,L,indMat,rho_vec,dx,betaX,betaY,Xreshape,Yreshape,ind);
thetalin = getLinearPar(Lo,reshapeobs,K*L,fitzeta,0); % Zeta must be subtracted before running function
output = outputStatSTEM([betaX, betaY, type],rho_vec,thetalin(1:end-fitzeta),0,dx,peakShape);
if nargin>=16
    output.GUI = GUI;
end
output = combinedLorentz(output, K, L, ind);
