function EstimatedParametersnonlin = fitAtomNonLinearLo(method,inCoordinates,obs_bs,X,Y,Box,betaX_estimatedbackground,betaY_estimatedbackground,rho_estimatedbackground,eta_estimatedbackground,dx,radius,options,ind)
...
for n=1:length(ind)
    ...
    for k = 1:length(betaX_b)
        ...
        combined_background = combined_background + eta_b(k)*(rho_b(k)^2 + R2).^(-3/2);
    end
    ...
    if strcmp(method,'same')
        ...
        EstimatedParametersnonlin(:,n) = lsqnonlin('criterionLorentz_samerho',StartParametersnonlin_s',[],[],options,reshapeX_s,reshapeY_s,K_s,L_s,obs_s_back,rho_estimatedbackground(i));
    else
        StartParametersnonlin_s = [betaX_estimatedbackground(i) betaY_estimatedbackground(i) rho_estimatedbackground(i)];
        EstimatedParametersnonlin(:,n) = lsqnonlin('criterionLorentz_diffrho',StartParametersnonlin_s',[],[],options,reshapeX_s,reshapeY_s,K_s,L_s,obs_s_back);
    end
end