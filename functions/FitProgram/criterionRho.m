function Fun = criterionRho(rho,Xreshape,Yreshape,K,L,reshapeobs,betaX,betaY,fitzeta,type,indMat,dx,ind,max_n,hwaitbar,abortbut,offwait,maxIter)
...
model = combinedGauss(betaX(ind), betaY(ind), rho_vec(ind), thetalin(1:end-fitzeta),Xreshape,Yreshape,indMat,dx,K,L,max_n);
if fitzeta
