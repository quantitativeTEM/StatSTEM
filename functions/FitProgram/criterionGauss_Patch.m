function [Fun, Jacobian] = criterionGauss_Patch(thetanonlin,X,Y,K,L,obs,type,fitzeta)
% criterionGauss_Patch - criterion to fit gaussian peak with same width
%
%   Criterion is used to fit multiple peak locations to an image. The width
%   is kept constant for atoms having the same type. This file is meant for
%   fitting all peaks in a selecting part of an image (patch).
%
%   syntax: [Fun, Jacobian] = criterionGauss_Patch(thetanonlin,X,Y,K,L,obs,type,fitzeta)
...
if nargout==2
    l_rho = length(rho);
    firstorderderivative = sparse(K*L,3*n_c+l_rho+fitzeta);
    derivativeThetalinToThetanonlin = sparse(n_c+fitzeta,2*n_c+l_rho);
    % For Jacobian
    derGaToThetanonlin1 = sparse(K*L,n_c+fitzeta);
    derGaToThetanonlin2 = sparse(K*L,n_c+fitzeta);
    derGaToThetanonlin3 = sparse(K*L,n_c+fitzeta);
    for i=1:n_c
        % BetaX
    ...
    for j=1:l_rho
        matrix3 = sparse(K*L,n_c+fitzeta);
        ind = (type')==j;
    ...
    firstorderder2 = firstorderderivative(:,2*n_c+l_rho+1:3*n_c+l_rho+fitzeta);
