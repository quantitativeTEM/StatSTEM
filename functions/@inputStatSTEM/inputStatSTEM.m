...
%--------------------------------------------------------------------------
    ...
    methods
        obj = addPeaks(obj,type)
        obj = autoAddTypes(obj,varargin)
        obj = autoAssignTypes(obj,varargin)
        obj = averageDistance(obj)
        obj = calibrateImage(obj)
        obj = changePeaks(obj,type)
        input = defineRho(input,Name,Value,varargin)
        obj = deletePeaks(obj)
        obj = devideIndices(obj)
        output = fitGauss(input)
        output = fitGauss_diffrho(input, rho, offset, maxwait)
        output = fitGauss_samerho(input, rho, offset, maxwait)
        output = fitGauss_small(input,rho)
