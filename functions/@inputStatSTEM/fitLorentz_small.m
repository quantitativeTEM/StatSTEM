function output = fitLorentz_small(input,rho)
% fitLorentz_small - program to fit Lorentzian peaks having to a STEM image
%
%   In this method, lorentzian peak are fitted all at the same time to a STEM
%   image. One can select to fit with a different width for each atom, or
%   the same width for each atom type. 
%
%   syntax: output = fitLorentz_small(input,FP)
...
if nargin<2
    ...
    if length(input.coordinates(:,1))~=length(rho)
        error('fitLorenz_small: rho should be defined for every coordinate')
    end
end
...
if input.widthtype == 0
    ...
    EstimatedParametersnonlin = lsqnonlin('criterionLorentz_diffrhoSmall',StartParametersnonlin',[],[],options,...
...
elseif input.widthtype == 1
    ...
    EstimatedParametersnonlin = lsqnonlin('criterionLorentz_samerhoSmall',StartParametersnonlin',[],[],options,...
    ...
    EstimatedParametersnonlin = lsqnonlin('criterionLorentz_userdefinedrhoSmall',StartParametersnonlin',[],[],options,...
